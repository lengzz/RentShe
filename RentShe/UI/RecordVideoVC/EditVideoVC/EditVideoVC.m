...
- (UIView*)generatingView
{
    ...
    if (!_generationView) {
        ...
        _generateProgressView.progressTintColor = kRGB_Value(0x0accac);
...
- (void)onVideoEffectBeginClick:(TXEffectType)effectType
{
    ...
    switch ((TXEffectType)_effectType) {
        case TXEffectType_ROCK_LIGHT:
            [_videoCutView startColoration:kRGB_Value(0xEC5F9B) alpha:0.7];
            break;
        case TXEffectType_DARK_DRAEM:
            [_videoCutView startColoration:kRGB_Value(0xEC8435) alpha:0.7];
            break;
        case TXEffectType_SOUL_OUT:
            [_videoCutView startColoration:kRGB_Value(0x1FBCB6) alpha:0.7];
            break;
        case TXEffectType_SCREEN_SPLIT:
            [_videoCutView startColoration:kRGB_Value(0x449FF3) alpha:0.7];
            break;
        default:
            break;
    }
...
-(void) onGenerateComplete:(TXGenerateResult *)result
{
    ...
    if (result.retCode == 0) {
        ...
        PreviewVideoVC* vc = [[PreviewVideoVC alloc] initWithCoverImage:videoInfo.coverImage videoPath:_videoOutputPath renderMode:RENDER_MODE_FILL_EDGE isFromRecord:NO];
