...
@interface VisitorVC ()<UITableViewDelegate,UITableViewDataSource>
{
    UILabel *_totalLab;
    NSInteger _index;
}
...
- (void)viewDidLoad {
    ...
    [self createNavbar];
    [self myTabV];
    _totalLab.text = [NSString stringWithFormat:@"总访客数量：%zd",self.visitorNum];
...
- (void)createNavbar
{
    [self.navigationItem setTitleView:[CustomNavVC setNavgationItemTitle:@"访客"]];
...
{
    _index = 1;
    NSDictionary *dic = @{@"user_id": self.userId,@"page":@(_index)};
    [NetAPIManager visitorList:dic callBack:^(BOOL success, id object) {
        ...
        if (success) {
            ...
            if ([arr isKindOfClass:[NSArray class]])
            {
                ...
                for (id obj in arr) {
                    VisitorM *model = [VisitorM new];
                    [model setValuesForKeysWithDictionary:obj];
                    [resArr addObject:model];
                }
                self.dataArr = [VisitorM packetFilter:self.dataArr andNewArr:resArr];
...
- (NSInteger)tableView:(UITableView *)tableView numberOfRowsInSection:(NSInteger)section
{
    ...
    NSMutableArray *arr = dic[kVisitorMContent];
    return arr.count;
}
